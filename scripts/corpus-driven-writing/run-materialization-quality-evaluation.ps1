param(
    ...
    [string]$Output = "build/tmp/corpus-driven-writing/materialization-quality",
...
if (-not $outputPath.StartsWith($allowedRoot + [System.IO.Path]::DirectorySeparatorChar, [System.StringComparison]::OrdinalIgnoreCase)) {
    throw "Quality evaluation output must stay under build/tmp/corpus-driven-writing."
}
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Materialization quality evaluation failed with exit code $LASTEXITCODE."
    }
}
finally {
    Pop-Location
