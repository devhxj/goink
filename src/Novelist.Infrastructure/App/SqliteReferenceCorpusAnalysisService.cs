...
public sealed class SqliteReferenceCorpusAnalysisService : IReferenceCorpusAnalysisService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var anchorNovelId = reader.IsDBNull(12) ? 0 : reader.GetInt64(12);
        if (anchorNovelId != 0 && novelId != 0 && anchorNovelId != novelId)
        {
            return null;
        }

        var persistedDiagnostics = SanitizeDiagnostics(diagnostics ?? ReadDiagnostics(reader.GetString(11)));
        return new ReferenceCorpusTechniqueSpecimenAnalysisRunPayload(
            ...
            Status: reader.GetString(3),
            TokensSpent: reader.GetInt32(4),
            SpecimenCount: reader.GetInt32(13),
            ProcessedNodes: processedNodes,
            AnalyzerVersion: reader.GetString(5),
            SchemaVersion: reader.GetString(6),
            ModelProvider: reader.GetString(7),
            ModelId: reader.GetString(8),
            StartedAt: DateTimeOffset.Parse(reader.GetString(9), CultureInfo.InvariantCulture),
