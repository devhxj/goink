...
Latest Phase 12 selected-row bulk corpus actions slice: `npm --prefix frontend run test:reference-anchor` passed after the corpus library list gained selected-row bulk promotion for per-novel anchors and selected-row bulk archive for workspace-corpus rows. The mocked browser workflow asserts selection counts, clearing processed selections, per-anchor `PromoteReferenceAnchorToWorkspaceCorpus`/`DeleteReferenceAnchor` payloads, and archived rows disappearing from the visible library. This remains a selected-row management slice, not bulk import, automatic migration, or bulk material editing.
