...
- [ ] Frontend smoke test for corpus management and default automatic material selection. Current coverage asserts default automatic material selection, separated corpus-library/reference-writing section headings, create/list/edit corpus metadata, owner-scope filtering/counts, local list query/license/source-trust filters, paginated row-level material browsing with local page filtering/sorting, single-material tag correction, single-anchor promotion, and workspace-corpus archive actions; full corpus library management UI smoke remains pending.
