...
- [x] The corpus library list now supports selected-row bulk promotion for per-novel anchors and selected-row bulk archive for workspace-corpus rows by reusing the existing safe bridge operations; the Playwright workflow asserts selection counts, processed-selection clearing, per-anchor bridge payloads, and archived rows disappearing from the visible library. This remains a selected-row management slice, not bulk import, automatic migration, or bulk material editing.
