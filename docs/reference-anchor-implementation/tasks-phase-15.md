...
**Status:** Complete. Resizable sidebar/chat panels, unified layout settings persistence, reload and compact viewport clamping, corrupted layout fallback, runtime window bounds capture, viewport/maximized state saving, and startup desktop x/y/width/height/maximized launch restore are implemented and covered. Restored desktop coordinates are clamped against Photino monitor work areas before launch to avoid off-screen windows.
...
- [x] Window size/position/maximized state persist through current Photino runtime APIs where available.
- [x] Restored window bounds are clamped to visible screen area to avoid off-screen windows.
...
- [x] Targeted Photino desktop test if window APIs are available in test harness.
...
- `src/Novelist.App/Desktop/PhotinoBridgeRuntimeHost.cs`
- `src/Novelist.App/Desktop/PhotinoWindowFactory.cs`
- `src/Novelist.App/Desktop/PhotinoWindowPlacement.cs`
...
- [x] Error state is cleared only on retry/success or explicit close, not accidentally by unrelated rerenders. Shared copyable callouts assert the callout remains visible after copy-feedback rerenders; reference-anchor create failure persists through unrelated form edits until explicit close; blueprint generate/review/approve failures clear after successful retry; and legacy character/location/story-arc/timeline/reader/preference/style-sample, novel dialog, editor save, skill edit, and legacy style extraction surfaces no longer clear errors from unrelated create/edit, reopen, or ordinary input actions.
...
**Status:** Complete as of 2026-07-08. README/README_EN now describe Phase 15 import/style/pattern/Git/update/Git-author capabilities; release notes summarize the user-facing changes and verification commands; Help/settings copy covers import recovery, encoding/size-limit rejection, update checks, Git author behavior, and style/pattern limitations; developer bridge docs record Phase 15 methods, guardrails, deferred scope, and the read-only legacy-reference rule for `goink-master`.

**Acceptance criteria:**

- [x] README/README_EN mention supported import formats, style material library, narrative pattern extraction, Git history, update checks, and configurable Git author identity after implementation lands.
- [x] Release notes summarize user-facing changes and verification commands.
- [x] Help/settings copy explains import cleanup/recovery behavior, encoding/size-limit rejection, update-check behavior, Git author settings, and style/pattern extraction limitations.
- [x] Developer docs record Phase 15 bridge methods, guardrails, and known deferred scope.
- [x] Docs explicitly keep `goink-master` as a read-only legacy reference, not an implementation target.

**Verification:**

- [x] Documentation links resolve.
- [x] `rg` confirms no new references to `frontend/src/lib/wailsjs`, Go/Wails build commands, or legacy app/internal implementation paths outside explanatory docs.
...
- [x] Tasks 16-23 complete.
- [x] Git history UI is read-only, paged, diff-capable, and robust for empty repos.
- [x] Git author identity, update checks, layout/window persistence, relative-time refresh, and visible copyable errors are implemented.

### Checkpoint E: Guardrails, QA, and Documentation

- [x] Tasks 24-27 complete.
- [x] Agent boundaries are explicit and tested.
- [x] Phase 15 Playwright suite and backend stress/fault-injection tests pass.
- [x] README/release/help docs match implemented behavior.

## Definition of Done

Phase 15 is complete only when all of the following are true:

- [x] EPUB/TXT/MD import works from file picker and drag/drop with encoding detection, progress, cancellation, skipped-chapter reporting, Git integration, and cleanup on failure.
- [x] TXT/MD import handles UTF-8, UTF-16 LE/BE with BOM, conservative UTF-16 no-BOM detection, and GB18030 without silent mojibake.
- [x] Import size limits are enforced with visible structured errors.
- [x] Startup import recovery reconciles incomplete import runs idempotently and blocks unsafe cleanup with diagnostics instead of guessing.
- [x] The import workflow preserves user data when only the post-import Git commit fails and reports that warning clearly.
- [x] Successful import triggers normal search/RAG indexing behavior but does not implicitly create reference anchors, style samples, or style profiles.
- [x] Style sample library supports CRUD, tags, global/per-novel filtering, pagination, deterministic stats, and LLM-backed validated skill extraction with cancellation.
- [x] Style sample behavior is aligned with Phase 14 style profiles and does not create a prompt-only bypass of style audit/provenance gates.
- [x] Narrative pattern extraction supports all chapters and multi-range selection, visible staged progress, validated LLM JSON, recursive compression safeguards, cancellation, trace inspection, and validated skill preview/save.
- [x] Git history panel supports paged commits, changed-file lists, renamed-file markers, lazy read-only diffs, empty repo handling, and cross-platform libgit2 runtime coverage.
- [x] Git author name/email can be configured and applied to new and existing repositories.
- [x] Sidebar/chat/window layout persistence works and is clamped safely.
- [x] Update checks are timeout-bounded, dismissible, manually retryable, testable without live network, and use explicit external URL opening.
- [x] CRUD/dialog failures no longer fail silently; copyable diagnostics exist and redact sensitive content.
- [x] Relative-time labels refresh while the app remains open.
